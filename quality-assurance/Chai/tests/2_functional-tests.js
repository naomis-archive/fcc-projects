...
suite("Functional Tests", function () {
      ...
      test('send {surname: "Colombo"}', function (done) {
        ...
        chai
          ...
          .end(function (err, res) {
            ...
            assert.equal(res.type, "application/json", "Should be a json");
            assert.equal(
              res.body.name,
              "Cristoforo",
              "First name should be Christoforo"
            );
            assert.equal(
              res.body.surname,
              "Colombo",
              "Last name should be Colombo"
            );
      ...
      test('send {surname: "da Verrazzano"}', function (done) {
        ...
        chai
          ...
          .end(function (err, res) {
            ...
            assert.equal(res.status, 200, "response status should be 200");
            assert.equal(res.type, "application/json", "Should be a json");
            assert.equal(res.body.name, "Giovanni", "First name Giovanni");
            assert.equal(
              res.body.surname,
              "da Verrazzano",
              "Last name da Verrazzano"
            );
            done();
          });
      });
  ...
  Browser.site = "https://pinnate-luxurious-mosquito.glitch.me/";
